...
/**
 * A utility class to read and analyze trade data from an Arrow file (.arrow) using memory-mapped I/O,
            ...
            try (RandomAccessFile raf = new RandomAccessFile(arrowFile, "r")) {
                try (FileChannel fileChannel = raf.getChannel()) {
                    try (ArrowFileReader reader = new ArrowFileReader(fileChannel, allocator)) {
                        ...
                        System.out.println("Reading " + recordBatches.size() + " record batches...");
                        for (ArrowBlock block : recordBatches) {
                            ...
                            totalBatchLoadNanos += (System.nanoTime() - t1);

                            int batchSize = root.getRowCount();
                            System.out.println("Loaded batch with " + batchSize + " rows...");
